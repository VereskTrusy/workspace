...
public class PaginationInfo {
	...
	private int totalRecord; // 전체 레코드 건수(조회할 데이터), 100건
	
	private int recordCount; // 한 페이지의 레코드 수(임의 결정), 10개
	
	private int pageSize; // 한 페이지에서 제공할 페이지 링크 수(임의 결정), 5개
	
	private int totalPage; // 전체 페이지 수, 10페이지
	
	private int currentPage; // 클라이언트 요구 페이지(요청 파라미터), 1
	
	private int firstRecord; // 현재 페이지의 첫번째 레코드 번호 , 1번
	
	private int lastRecord; // 현재 페이지의 마지막 레코드 번호 , 10번
	
	private int firstPage; // 한 구간에서 제공할 첫번째 페이지 번호 , 1페이지
	
	private int lastPage; // 한 구간에서 제공할 마지막 페이지 번호 , 5 페이지
